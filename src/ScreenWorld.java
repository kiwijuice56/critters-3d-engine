...
public class ScreenWorld extends ActorWorld {
	...
	public void fillTriangle(Triangle tri) {
		...
		double du1 = (t2.x - t1.x) / scanLines;
		double du2 = (t4.x - t1.x) / scanLines;
		double dv1 = (t2.y - t1.y) / scanLines;
		double dv2 = (t4.y - t1.y) / scanLines;
		...
		//System.out.println("u " + du1 + " " + du2 + " v " + dv1 + " " + dv2 + " " + (t4.x - t1.x));
		...
		for (double y = Math.ceil(v1.y); y < v2.y; y++) {
			...
			for (double x = x1; x < x2; x++) {
				...
				if (z > depthBuffer[(int) y][(int) x]) {
					...
					setPixelColor((int) x, (int) y, Triangle.blendColor(new Color(argb), getDrawColor()));
				}
			}
		...
		du1 = (t2.x - t3.x) / scanLines;
		du2 = (t4.x - t3.x) / scanLines;
		dv1 = (t2.y - t3.y) / scanLines;
		dv2 = (t4.y - t3.y) / scanLines;
		...
		for (double y = Math.floor(v3.y); y > v4.y; y--) {
			...
			for (double x = x1; x < x2; x++) {
				...
				if (z > depthBuffer[(int) y][(int) x]) {
					...
					setPixelColor((int) x, (int) y, Triangle.blendColor(new Color(argb), tri.getColor()));
